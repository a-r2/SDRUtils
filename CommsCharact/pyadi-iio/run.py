...
for ind_sr, cur_sr in enumerate(sample_rates):
    sdr.sample_rate = cur_sr
    # Check if current sample rate is indeed set in the SDR
    if np.mod(sdr.sample_rate, cur_sr) < 1:
        print("Testing sample rate: " + str(cur_sr * 1e-6) + " MHz...")
    while not np.mod(sdr.sample_rate, cur_sr) < 1:
        sdr.sample_rate = cur_sr
    for ind_bs, cur_bs in enumerate(buffer_sizes):
        if cur_bs > cur_sr: #ignore when buffer size is greater than sample rate
            break
        sdr.rx_buffer_size = cur_bs
        # Check if current buffer size is indeed set in the SDR
        if np.mod(sdr.rx_buffer_size, cur_bs) < 1:
            print("Testing buffer size: " + str(cur_bs) + " samples...")
        while not np.mod(sdr.rx_buffer_size, cur_bs) < 1:
            sdr.rx_buffer_size = cur_bs
        diff_time = np.NaN * np.ones(ITER_NUM) #initialize array
        for i in range(ITER_NUM):
            start_time   = time.perf_counter()
            iq           = sdr.rx() #get buffer size samples
        ...
        sdr.rx_destroy_buffer() #destroy buffer in order to change its size
# Transform units
latency    *= 1e3 #t -> ms
jitter     *= 1e3 #t -> ms
